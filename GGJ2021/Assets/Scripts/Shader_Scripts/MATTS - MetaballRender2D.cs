//Matt wrote this out but something is wriong with it
...
public class MetaballRender2D : ScriptableRendererFeature
{
    ...
    class MetaballRender2DPass : ScriptableRenderPass
    {
        ...
        {
            //this buffer store the commands that make up the pass, its the list of insructions for the GPU
            CommandBuffer cmd = CommandBufferPool.Get(profilerTag);

            //more strange fixed array stuff as we have to pass this file to the GPU
            //send this fake array of metaballs to fix the array size on the GPU
            if (isFirstRender)
            {
                isFirstRender = false;
            ...
            //Now lets get the real metaball data from our static class
            List<Metaball2D> metaballs = MetaballSystem2D.Get();
            List<Vector4> metaballData = new List<Vector4>(metaballs.Count);

            //for each of those metaballs add them to our data packet
            ...
            //if theres at least one metaball lets add the commands to the buffer
            if(metaballData.Count > 0)
            {
                cmd.SetGlobalInt("_MetaballCount", metaballs.Count);
                cmd.SetGlobalVectorArray("MetaballData", metaballData);
